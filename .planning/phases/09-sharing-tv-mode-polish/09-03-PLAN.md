...
- Define `MilestoneData` interface:
  ...
  ```
- **Species recovery milestone:** For each species with `status === 'recovered'` or `status === 'stabilized'` (lowercase — matching the `SpeciesRecovery.recoveryStatus` enum), create key `species:${name}`. If not in `celebrated` set, add it and call `celebrate('milestone')`.
...
if (SITE_VARIANT === 'happy' && speciesData) {
  checkMilestones({
    speciesRecoveries: speciesData.map(s => ({ name: s.commonName, status: s.recoveryStatus })),
...
```typescript
// After: this.renewableEnergyPanel?.setData(data);
